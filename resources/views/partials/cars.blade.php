<section class="w-full max-w-[1440px] p-4">
    @if($cars == null || count($cars) == 0)
        ...
        @if(request()->is('/'))
            <p class="italic text-center">No cars available. <a href="{{ route('cars') }}" class="!not-italic hover:opacity-90 hover:underline active:opacity-85">See all cars here.</a></p>
        @else
            @if(session()->get('role_id') == 1)
                <p class="italic text-center">No cars on database. <a href="{{ route('add') }}" class="!not-italic hover:opacity-90 hover:underline active:opacity-85">Add car here.</a></p>
            @else
                <p class="italic text-center">No cars on the list. <a href="{{ route('contact') }}" class="!not-italic hover:opacity-90 hover:underline active:opacity-85">Contact us here.</a></p>
            @endif
        @endif
    @else
